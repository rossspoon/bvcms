...
BEGIN
	BEGIN TRY 
		...
		DELETE FROM dbo.PeopleExtra WHERE PeopleId = @pid
		DELETE FROM dbo.Coupons WHERE PeopleId = @pid
		DELETE FROM dbo.TransactionPeople WHERE PeopleId = @pid
		DELETE FROM dbo.EmailOptOut WHERE ToPeopleId = @pid
		DELETE FROM dbo.EmailResponses WHERE PeopleId = @pid
		DELETE FROM dbo.EmailQueueTo WHERE PeopleId = @pid;
		DELETE dbo.EmailQueueTo FROM dbo.EmailQueueTo et 
		JOIN EmailQueue e ON e.Id = et.Id
		WHERE QueuedBy = @pid
		DELETE FROM dbo.EmailQueue WHERE QueuedBy = @pid;
