...
-- =============================================
CREATE PROCEDURE [dbo].[QueuePriorityEmail]
	(@id INT, @CmsHost varchar(50), @Host varchar(50))
AS
BEGIN

	DECLARE @conversation UNIQUEIDENTIFIER
	EXEC CMSEmailQueue.dbo.CreateNewPriorityConversation @conversationID = @conversation OUTPUT;
	DECLARE @msg VARCHAR(200) = 'START|' + @Host + '|' + @CmsHost + '|' + CONVERT(VARCHAR(10), @id)
	EXEC CMSEmailQueue.dbo.SendMessage @conversation, @msg
	DECLARE @pid int
	DECLARE c1 CURSOR READ_ONLY FOR SELECT PeopleId FROM dbo.EmailQueueTo WHERE Id = @id
	OPEN c1
