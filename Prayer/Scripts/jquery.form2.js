...
jQuery.fn.formSerialize = function() {
    return $.param(this.formToArray());
};
jQuery.AddQuery = function(q, extra) {
}
jQuery.populateForm = function(qs) {
    var qa = new Array();
    var p = qs.split("&");
    for (i in p) {
        var a = p[i].split("=");
...
jQuery.QueryString = function(q, item) {
    var r = new Object();
...
jQuery.navigate = function(url, data) {
    url += (url.match(/\?/) ? "&" : "?") + data;
    window.location = url;
};
