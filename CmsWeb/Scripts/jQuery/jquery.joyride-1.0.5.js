...
(function ($) {
	$.fn.joyride = function (options) {
		...
		var settings = {
			...
			'postRideCallback': $.noop, // A method to call once the tour completes
			'postCancelCallback': $.noop, // A method to call once the tour is canceled
      ...
      timerIndicatorTemplate = '<div class="joyride-timer-indicator-wrap"><span class="joyride-timer-indicator"></span></div>';
        ...
        opt = {};
          ...
          nubHeight = Math.ceil($('.joyride-nub').outerHeight() / 2);
			...
			var cancelTip = function (e, interval_id, cookie, self) {
				e.preventDefault();
				clearInterval(interval_id);
				if (cookie) {
					$.cookie(settings.cookieName, 'ridden', { expires: 365, domain: settings.cookieDomain });
				...
				$(self).parent().parent().hide();
				if (settings.postCancelCallback != $.noop) {
					settings.postCancelCallback();
				}
			}

			$('.joyride-close-tip').click(function (e) {
				cancelTip(e, interval_id, settings.cookieMonster, this);
			});
