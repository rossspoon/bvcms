...
<p>{1:ddd MMM d, yyyy h:mm tt} to {3:h:mm tt}</p>".Fmt(m.person.PreferredName.TrimEnd(), m.meeting.MeetingDate, name, m.meeting.MeetingDate.Value.AddMinutes(90)));
            var stepmail = DbUtil.Db.Setting("StepMail", DbUtil.SystemEmailAddress);
            var p = DbUtil.Db.People.SingleOrDefault(pp => pp.EmailAddress == stepmail || pp.EmailAddress2 == stepmail);
            DbUtil.Db.Email(m.email, p, "Step Class Registration", @"{0}({1}) registered in the {3} Class for the following date:
