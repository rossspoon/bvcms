...
namespace CMSWeb.Models
{
    public class RecDetailModel
    {
        ...
        public bool NoReassign { get; set; }
        ...
        public string AgeDivActual
        {
            get
            {
                if (recreg.Person == null)
                    return "no person";
                var q = from om in recreg.Person.OrganizationMembers
                        where om.Organization.DivOrgs.Any(dd => dd.DivId == recreg.DivId)
                        select om.Organization.OrganizationName;
                var s = string.Join(";", q.ToArray());
                if (!s.HasValue())
