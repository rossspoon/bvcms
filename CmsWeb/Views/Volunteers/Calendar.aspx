    ...
    $(function() {
        $(".reminder").click(function() {
            if (confirm("are you sure?")) {
                $.blockUI();
                $.post("/Volunteers/EmailReminders/" + $(this).attr("mid"));
                $.unblockUI();
            }
        });
    });
	...
	<tbody>
		...
		<tr>
            ...
            <td valign="top">
                <% if (cell != null)
                   { %>
                <a href="#" class="reminder" mid="<%=cell.MeetingId %>">Email Reminders(<%=cell.Persons.Count() %>)</a><br /><br />
                   <% foreach (var p in cell.Persons)
                      { %>
                <a href="/Person.aspx?id=<%=p.PeopleId %>"><%=p.Name%></a><br />
                   <% } %>
                <% } %>
            </td>
		    <% } %>
